...
from peaktk.preprocessing import helper
...
def load_data(DEMAND_PATH, WEATHER_PATH, years=[2018, 2019, 2020]):
    ...
    dt_list = helper.convert_string_to_datetime(isone_df)
    isone_df["dt"] = dt_list
    ...
    daily_df = helper.convert_hourly_to_daily_data(merge_df)
...
def load_data_hourly(DEMAND_PATH, WEATHER_PATH, years=[2018, 2019, 2020]):
    # load ISONE dataset
    path = DEMAND_PATH
    all_df = []
    for y in years:
